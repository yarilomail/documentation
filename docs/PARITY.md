...
| Virtual mailboxes | partial | a namespace whose mailboxes are defined by a configuration file of folders and `SEARCH` rules; kept in sync while open, with `FETCH`, `SEARCH`, `SORT`, `THREAD`, `STORE` and `EXPUNGE` acting on the real message and `APPEND`/`COPY`/`MOVE` into one storing to its save folder. `MOVE` out of one and the `+` and annotation line forms are not implemented yet. See [Virtual Mailboxes](/VIRTUAL) |
